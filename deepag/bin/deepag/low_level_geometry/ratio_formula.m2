...
---- K1, K2 diagonal
clearAll
R = QQ[r,f2,f11,f12,f13,f21,f22,f23,f31,f32,f33, MonomialOrder=>Lex]
F = matrix{{f11,f12,f13},{f21,f22,f23},{f31,f32,f33}}
K1 = matrix{{f2*r, 0, 0}, {0, f2*r, 0}, {0, 0, 1}}
...
Gs = saturate(G,ideal(r*f2)); -- det(K1),det(K2) are non-zero
-- Gs = Gs + ideal(f33-1)
dim Gs, codim Gs, degree Gs
M = eliminate(Gs)),{r,f2}); -- Constraints on F only
dim M, codim M, degree M
m = mingens M -- = -det(F)
g = mingens gb Gs
s2 = mingens gb eliminate(Gs,r)
s1 = mingens gb eliminate(Gs,f2)
-- Formula for r
(m11,c11) = coefficients(s1_1_0,Variables=>{r}) -- extract coefficients 
-- Formula for f2

...
% 3D points
X = 2*sc*2*Fparam.points+repmat([-sc;-sc;2*sc],1,size(Fparam.points,2));
...
% Rotations
R1 = eye(3);
R2 = a2r(Fparam.raxis,pi/8);
% Projection matrices
P1 = K1*R1*[eye(3)     [-sc;0;0]];
P2 = K2*R2*[eye(3)  sc*[Fparam.tpos;0]];
% Image points
u1 = X2u(X,P1);
u2 = X2u(X,P2);
% add image noise
...
%toc();
%tic();
[F,A]  = uu2F({u1,u2},{'[-1,1]',method}); % from image measurements
for i=1:size(F,3)
    F(:,:,i) = F(:,:,i)/norm(F(:,:,i));

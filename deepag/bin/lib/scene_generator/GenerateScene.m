...
function [P M m mnoiseless npoints] = GenerateScene(npoints, radius, ncams, minz, maxz, noutliers, anoise, camcalib, sceneType, pointsseed, camseed, rdcoefs, removeBehind)
    ...
    elseif strncmp(motionType, 'xyplaneDepth', 11)
    ...
    elseif strncmp(motionType, 'xzplane', 6)
 ...
 elseif strncmp(motionType, 'sum_side', 7)
        ...
        for i = 1:ncams
            ...
            P{i} = SetLookAt(pos', at', [0 1 0]);
            P{i}(:,1:3) = P{i}(:,1:3)';
            %Re = rotvec(randn(3,1),180/pi/10000);
            Re = rotvec(randn(3,1),1/100*(pi/180));
            %Re = rotvec(randn(3,1),180/pi/10000000);
            P{i} = P{i}*Re;
            %P{i} = P{i}+0.000001*randn(3,4);
     ...
     elseif strncmp(motionType, 'forward', 7)
        ...
        for i = 1:ncams
            ...
            elseif strncmp(motionType, 'sum_forward', 9)
        ...
        for i = 1:ncams
            ...
            pos = dir * ((randn * (maxz - minz)) + minz) + [rand(1);rand(1);0];
            at = radius(2) * (randn(3, 1) - 0.5);
            P{i} = SetLookAt(pos', at', [0 1 0]);
            P{i}(:,1:3) = P{i}(:,1:3)';
    ...
    for j=1:ncams
        ...
        switch noiseModel
            case 0
                ...
                amp = ( anoise(j) - 1/3*anoise(j)*randn(1, npoints) );
            ...
            case 3
                p = m{j} + (randn(2,npoints))*anoise(j);

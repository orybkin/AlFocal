% [P,e,T] = uX2P(u,X[,cType,E]) - camera projection matrix P from 3D points X and their projections u
%
% cType = 'perspective' (implicit) or 'affine' ... estimates the standard cameras
%
%            u     ... 3 x n image points, affine coords for finite points
%            X     ... 3(4) x n (extended affine) coordinates of 3D point X
%            P     ... 3 x 4 matrix
%            e     ... 2 x n residuals
%
...
if nargin>0
    ...
    switch ct
        case 'perspective'
            ...
            P = B\(Q*A);
            P = P/vnorm(P(3,1:3));
            P = P * median(median(sign((P*X)./u))); % make P to project maximum of points on positive half-rays
            e = h2a(P*X)-h2a(u);
            T = eye(4);
        case 'affine'
            A           = eye(4);
    ...
    if false
        ...
        for m=2:2
            uu = []; ee = []; pp = []; tt = []; ww = [];
            for k=1:400 % Monte Carlo Sampling
                for i=1:length(u)

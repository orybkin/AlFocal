function [a,Q] = fitellipse(y)

% FITELLIPSE  Least-squares fit of ellipse to 2D points.
%        [A,Q] = FITELLIPSE(y) returns the parameters of the best-fit
%        ellipse to 2D points y(1:2,:).
...
if sx<1e-8, a=zeros(1,5); Q=zeros(3); return; end
if sy<1e-8, a=zeros(1,5); Q=zeros(3); return; end
...
% conic matrix
T = [ cos(thetarad)  sin(thetarad) -cos(thetarad)*uCentre-sin(thetarad)*vCentre
     -sin(thetarad)  cos(thetarad)  sin(thetarad)*uCentre-cos(thetarad)*vCentre
     0              0              1];
 Q = T'*diag([1/Ru^2 1/Rv^2 -1])*T;
 Q = Q/norm(Q);
